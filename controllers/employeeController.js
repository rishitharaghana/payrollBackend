...
const updateEmployee = async (req, res) => {
  upload.fields([{ name: "photo", maxCount: 1 }])(req, res, async (err) => {
    ...
    const userRole = req.user.role;
    const { id } = req.params;
    const { name, email, mobile, emergency_phone, address, role, blood_group } = req.body;
    const photo = req.files?.["photo"]?.[0];
    ...
    try {
      const [existingRecord] = await queryAsync(`SELECT * FROM ${table} WHERE id = ?`, [id]);
      if (!existingRecord) {
      ...
      const [mobileCheck] = await queryAsync(
        ...
        [mobile.trim(), id, mobile.trim(), id, mobile.trim(), id, mobile.trim(), id, mobile.trim(), id]
      );
      if (mobileCheck) {
        return res.status(400).json({ error: "Mobile number already in use" });
      }
      ...
      const photo_url = photo ? `${baseUrl}${path.basename(photo.path)}` : req.body.photo === "null" ? null : existingRecord.photo_url;

      const query = `UPDATE ${table} SET full_name = ?, email = ?, mobile = ?, emergency_phone = ?, address = ?, blood_group = ?, photo_url = ? WHERE id = ?`;
      const values = [name, email, mobile, emergency_phone || null, address || null, blood_group || null, photo_url, id];
      ...
      res.json({
        ...
        data: { id, role, name, email, mobile, emergency_phone, address, blood_group, photo_url },
      });
    } catch (err) {
      console.error("DB error:", err.message, err.sqlMessage, err.code);
...
const createDocuments = async (req, res) => {
  upload.fields([{ name: "document", maxCount: 1 }])(req, res, async (err) => {
    ...
    try {
      const [employee] = await queryAsync(`SELECT employee_id FROM employees WHERE employee_id = ?`, [employeeId]);
      if (!employee) {
        return res.status(404).json({ error: "Employee not found" });
      }
      ...
      res.status(201).json({
        ...
        data: { id: result.insertId, employee_id: employeeId, document_type: documentType, file_path: filePath },
      });
    } catch (err) {
      console.error("DB error:", err.message, err.sqlMessage, err.code);
      res.status(500).json({ error: `Database error: ${err.message}` });
...
const fetchEmployees = async (req, res) => {
  try {
    ...
    const deptHeads = await queryAsync(
      `SELECT id, employee_id, full_name, email, mobile, department_name, designation_name, 'dept_head' as role,
              CASE WHEN photo_url IS NOT NULL THEN CONCAT(?, photo_url) ELSE NULL END as photo_url
       FROM dept_heads`,
      [baseUrl]
    );
    const managers = await queryAsync(
      `SELECT id, employee_id, full_name, email, mobile, department_name, designation_name, 'manager' as role,
              CASE WHEN photo_url IS NOT NULL THEN CONCAT(?, photo_url) ELSE NULL END as photo_url
       FROM managers`,
      [baseUrl]
    );
    const employees = await queryAsync(
      `SELECT id, employee_id, full_name, email, mobile, department_name, designation_name, employment_type, basic_salary, allowances, join_date, 'employee' as role,
              CASE WHEN photo_url IS NOT NULL THEN CONCAT(?, photo_url) ELSE NULL END as photo_url
       FROM employees`,
      [baseUrl]
    );
...
const deleteEmployee = async (req, res) => {
  ...
  try {
    const [existingRecord] = await queryAsync(`SELECT photo_url FROM ${table} WHERE id = ?`, [id]);
    if (!existingRecord) {
