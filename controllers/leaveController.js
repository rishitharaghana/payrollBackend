...
const calculateLeaveDays = async (startDate, endDate) => {
  ...
  try {
    ...
    holidayDates = holidays.map(
      (h) => h.holiday_date.toISOString().split("T")[0]
    );
  } catch (err) {
    console.error(
      "Error querying holidays table:",
...
const applyLeave = async (req, res) => {
  ...
  if (role === "super_admin") {
    return res
      .status(403)
      .json({ error: "Super admins cannot apply for leaves" });
  }
  ...
  if (!["Paid", "Unpaid"].includes(leave_status)) {
    return res
      .status(400)
      .json({ error: "Invalid leave status. Must be 'Paid' or 'Unpaid'" });
  }
  ...
  try {
    ...
    if (existingLeaves.length > 0) {
      return res
        .status(400)
        .json({
          error: "You already have a leave applied for the selected dates",
        });
    }

    let expectedRecipientRole;
    if (role === "employee" || role === "dept_head" || role === "hr") {
      expectedRecipientRole = "hr";
    } else {
      expectedRecipientRole = "super_admin";
    }
    ...
    if (!recipient) {
      return res
        .status(400)
        .json({
          error: `Recipient must be an active ${expectedRecipientRole}`,
        });
    }
...
const updateLeaveStatus = async (req, res) => {
  ...
  if (!leave_id || !status || !["Approved", "Rejected"].includes(status)) {
    return res
      .status(400)
      .json({
        error: "Leave ID and valid status (Approved/Rejected) are required",
      });
  }
  ...
  if (!["super_admin", "hr", "dept_head"].includes(role)) {
    return res
      .status(403)
      .json({ error: "Unauthorized to update leave status" });
  }

  try {
    let recipientCheckQuery =
    ...
    if (role === "dept_head") {
      ...
      recipientCheckQuery +=
        " AND EXISTS (SELECT 1 FROM leaves l JOIN hrms_users u ON l.employee_id = u.employee_id WHERE l.id = ? AND u.department_name = ?)";
      recipientCheckParams.push(leave_id, deptHead.department_name);
    } else if (role === "super_admin") {
      // Restrict super_admin to HR-submitted leaves
      recipientCheckQuery +=
        " AND EXISTS (SELECT 1 FROM leaves l JOIN hrms_users u ON l.employee_id = u.employee_id WHERE l.id = ? AND u.role = 'hr')";
      recipientCheckParams.push(leave_id);
    }

    const recipientCheck = await queryAsync(
      recipientCheckQuery,
      recipientCheckParams
    );
    if (!recipientCheck.length) {
      return res
        .status(403)
        .json({ error: "You are not authorized to update this leave" });
    }
    ...
    if (status === "Approved") {
      ...
      for (
        ...
        try {
          ...
          console.error(
            "Error querying holidays in updateLeaveStatus:",
            err.sqlMessage || err.message
          );
          try {
            await queryAsync(
              ...
              [
                "updateLeaveStatus",
                "error",
                `Failed to query holidays: ${err.sqlMessage || err.message}`,
              ]
            );
          } catch (logErr) {
        ...
        if (holidayDates.length === 0 && d.getDay() !== 0 && d.getDay() !== 6) {
          await queryAsync(
            ...
            [
              leave.employee_id,
              dateStr,
              leave.leave_status === "Paid" ? "Approved" : "Unpaid",
              employee_id,
              "Leave",
            ]
          );
...
const getRecipientOptions = async (req, res) => {
  ...
  try {
    ...
    if (role === "employee" || role === "dept_head") {
      ...
      return res
        .status(403)
        .json({ error: "Invalid role for fetching recipients" });
    }

    if (recipients.length === 0) {
      return res
        .status(404)
        .json({ error: "No valid recipients found for your role" });
    }

    const formattedRecipients = recipients.map((recipient) => ({
      value: recipient.identifier, // employee_id for HR, full_name for Super Admin
      label: `${recipient.full_name} (${
        recipient.role === "super_admin" ? "Super Admin" : "HR"
      })`,
      role: recipient.role,
    }));
...
const allocateMonthlyLeaves = async (req = {}, res = null) => {
  try {
    console.log("allocateMonthlyLeaves - Request body:", req.body);
    ...
    console.log(
      `allocateMonthlyLeaves - Year: ${currentYear}, Month: ${currentMonth}`
    );
    ...
    if (
      ...
      if (res) return res.status(400).json({ error: errorMessage });
    ...
    if (!employees.length) {
      const message =
        "No active employees, HR, managers, or department heads found for leave allocation";
      console.log(message);
      await queryAsync(
        "INSERT INTO cron_logs (job_name, status, message, executed_at) VALUES (?, ?, ?, ?)",
    ...
    for (const employee of employees) {
      ...
      try {
        ...
        if (existingBalance.length > 0) {
          ...
          console.log(
            `Allocated ${monthlyAllocation} ${leaveType} leave for ${role} ${employee_id}`
          );
          allocatedCount++;
        } else {
          await queryAsync(
          ...
          console.log(
            `Initialized ${leaveType} leave with ${monthlyAllocation} for ${role} ${employee_id}`
          );
          allocatedCount++;
        }
      } catch (err) {
        console.error(
          `Failed to allocate leave for ${role} ${employee_id}:`,
          err
        );
    ...
    if (allocatedCount === 0) {
      const errorMessage = `Failed to allocate leaves for ${currentYear}-${currentMonth}: No allocations succeeded. Errors: ${JSON.stringify(
        failedEmployees
      )}`;
      await queryAsync(
        "INSERT INTO cron_logs (job_name, status, message, executed_at) VALUES (?, ?, ?, ?)",
      ...
      if (res)
        return res
          .status(500)
          .json({
            error: "Failed to allocate monthly leaves",
    ...
    const message = `Monthly leave allocation for ${currentYear}-${currentMonth} completed. Allocated for ${allocatedCount} users.${
      failedEmployees.length > 0
        ? ` Failed for: ${JSON.stringify(failedEmployees)}`
        : ""
    }`;
    await queryAsync(
      "INSERT INTO cron_logs (job_name, status, message, executed_at) VALUES (?, ?, ?, ?)",
    ...
    const errorMessage = `Failed to allocate leaves for ${
    ...
    if (res)
      return res
        .status(500)
        .json({ error: errorMessage, details: err.sqlMessage || err.message });
    throw err;
  }
};
  ...
  try {
    ...
    if (!validLeaveTypes.includes(leave_type)) {
      return res
        .status(400)
        .json({
          error: "Invalid leave type. Must be 'maternity' or 'paternity'",
        });
    }
    ...
    if (existingBalance) {
      return res
        .status(400)
        .json({
          error: `${leave_type} leave already allocated for ${currentYear}`,
        });
    }
    ...
    res
      .status(201)
      .json({ message: `${leave_type} leave allocated successfully` });
  } catch (err) {
    console.error(
      `Error in allocateSpecialLeave for ${leave_type}:`,
      err.sqlMessage || err.message
    );
    res
      .status(500)
      .json({
        error: `Failed to allocate ${leave_type} leave`,
        details: err.sqlMessage || err.message,
      });
  }
};
